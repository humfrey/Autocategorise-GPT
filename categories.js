const industryCategoriesSEO = {
  "Material Handling Equipment": {
    "description": "",
    "categories": [
      "Pallet Racks For Sale",
      "Forklift Truck Equipment",
      "Conveyors For Sale",
      "Machinery & Tools For Sale",
      "Carton Flow Racks, Case Flow Racks, Pick Module",
      "Material Handling Attachments",
      "Warehouse Storage Rack",
      "Other"
    ]
  },
  "Manufacturing & Machinery": {
    "description": "",
    "categories": [
      "Metalworking Equipment",
      "Milling Machines",
      "Woodworking Equipment",
      "Metalworking Lathes",
      "Pharmaceutical Equipment",
      "Wood CNC Machines",
      "Food & Beverage Equipment",
      "Brewery Equipment",
      "Plastics & Rubber Manufacturing Equipment",
      "Food & Beverage Dryers",
      "Processing & Packaging Equipment",
      "Other"
    ]
  },
  "Restaurant & Bar Equipment": {
    "description": "",
    "categories": [
      "Commercial Cooking Equipment",
      "Commercial Restaurant Ranges",
      "Commercial Refrigerators & Equipment",
      "Commercial Grills",
      "Food Prep Equipment",
      "Commercial Ovens",
      "Commercial Sinks",
      "Concession Equipment",
      "Food Warmers & Holding Equipment",
      "Commercial Exhaust Hoods",
      "Restaurant Food Storage & Transport",
      "Outdoor Cooking Equipment",
      "Beverage Equipment",
      "Meat Processing Equipment",
      "Food Display & Merchandising",
      "Commercial Fryers",
      "Tabletop Supplies",
      "Restaurant Seating & Furniture",
      "Other"
    ]
  },
  "Trucks & Vehicles": {
    "description": "",
    "categories": [
      "Commercial Truck Auctions",
      "Cars & Vehicle Auctions",
      "Asphalt & Concrete Trucks",
      "Aircraft",
      "Bucket Trucks",
      "ATVs",
      "Emergency Vehicles",
      "Automobiles",
      "Forestry Trucks",
      "Classic Car Auctions",
      "Hauling & Transport Trucks",
      "Bus Auctions",
      "Service & Utility Trucks",
      "Marine Vehicles",
      "Specialty Truck",
      "Motorcycle Auctions",
      "Tow & Recovery Trucks",
      "Pickup Truck Auctions",
      "Truck Tractors",
      "Recreational Vehicles",
      "Van & Cargo Trucks",
      "SUVs",
      "Waste & Water Trucks",
      "Vans",
      "Truck Trailer",
      "Auto Parts & Accessories",
      "Other"
    ]
  },
  "Construction & Mining": {
    "description": "inc Residential and Commercial Building Supplies",
    "categories": [
      "Construction Aggregate",
      "Loaders",
      "Asphalt & Concrete Equipment",
      "Off Road Trucks",
      "Compactors",
      "Skid Steers",
      "Cranes",
      "Sweepers & Brooms",
      "Bulldozers",
      "Light Equipment & Support",
      "Drilling & Mining Equipment",
      "Trenchers",
      "Excavators",
      "Building Materials",
      "Forestry & Logging Equipment",
      "Construction Trailers",
      "Lifts",
      "Construction Equipment Attachments",
      "Crushers",
    "Pumps",
    "Concrete Support Equipment",
    "Tanks & Totes",
    "Rollers",
    "Tools",
    "Crane Parts & Accessories",
    "Winches, Come Alongs & Straps",
    "Oilfield Equipment",
    "Skid Steer Attachments",
    "Log Loaders",
    "Equipment Attachments",
    "Aerial Lifts",
    "Excavator Attachments",
    "HVAC Equipment",
    "Blade & Scraper Attachments",
    "Landscape & Commercial Lawncare",
      "Other"
    ]
  },
  "Printing Equipment": {
    "description": "",
    "categories": [
      "Printing Equipment",
      "Bindery and Finishing",
      "Consumables",
      "Mailing Equipment",
      "Printers & Presses",
      "Pre-Press Equipment",
      "Other"
    ]
  },
  "Test & Measurement": {
    "description": "",
    "categories": [
      "Test & Measurement Equipment",
      "Analyzers",
      "Wave Generators",
      "Materials Testing",
      "Optics / Photonics Equipment",
      "Test & Measurement - General",
      "Analytical Instrument",
      "Other"
    ]
  },
  "Medical & Lab Equipment": {
    "description": "",
    "categories": [
      "Medical & Lab Equipment",
      "Diagnostic Instrument",
      "Imaging Equipment",
      "General Medical Equipment",
      "Surgical Equipment",
      "Other"
    ]
  },
  "Farm Equipment": {
    "description": "",
    "categories": [
      "Agriculture Technolog",
      "Combines & Harvesters",
      "Applicators & Sprayers",
      "Headers & Platforms",
      "Grain & Feed Handling Equipmen",
      "Hay Balers & Equipment",
      "Harvest Equipment",
      "Hay & Forage Harvesting Equipment",
      "Hay & Forage Equipment",
      "Hay & Forage Mowers",
      "Livestock Supplies",
      "Seeders",
      "Planters & Seeding",
      "Tractor Parts",
      "Tillage Equipment",
      "Tractor Attachments",
      "Tractor",
      "John Deere Tractors",
      "Farm Wagons & Trailers",
      "Other"
    ]
  },
  "Computers & Electronics": {
    "description": "",
    "categories": [
      "Audio / Visual Equipment",
      "Speakers",
      "Phones",
      "TVs",
      "Computer Auctions",
      "Broadcasting Equipment",
      "Professional Lighting Equipment",
      "Laptops",
      "Networking & Communications Equipmen",
      "Photography & Video Equipment",
      "Printers & Scanners",
      "Security & Surveillance Equipment",
      "Other"
    ]
  }
};

const industryCategoriesMeganav = {
  "Industrial & Commercial Equipment": {
    description: "where not covered by other categories",
    categories: [
      "Air Compressors",
      "Electrical Supplies",
      "Forklift Truck Equipment",
      "Power Generators",
      "Material Handling Equipment",
      "Other Industrial Supplies & Equipment",
      "Pipes & Tubing",
      "Scissor Lifts",
      "Warehouse Equipment",
      "Other",
    ],
  },
  "Metalworking Machinery": {
    description: "",
    categories: [
      "EDM Machines",
      "Misc. Machine Tools",
      "Metalworking Machinery",
      "Inspection / QA / Measuring / Testing",
      "Presses",
      "Metalworking Saws",
      "Sheet Metal Processing",
      "Surface Treatment Machinery",
      "Welding Equipment",
      "Lathes",
      "Milling Machines",
      "Other",
    ],
  },
  "Food & Beverage Equipment": {
    description: "",
    categories: [
      "CIP Systems",
      "Complete Plants",
      "Commercial Cooking Equipment",
      "Food Prep Equipment",
      "Restaurant Seating & Furniture",
      "Kitchen Equipment",
      "Industrial Mixers & Blenders",
      "Food Processing",
      "Commercial Refrigerators & Equipment",
      "Restaurant & Bar Equipment",
      "Tanks & Vessels",
      "Warewashing",
      "Food Display & Merchandising",
      "Other",
    ],
  },
  "Woodworking Equipment": {
    description: "",
    categories: [
      "Woodworking CNC Machines",
      "Doweling Machinery",
      "Milling, Morticing & Turning",
      "Panel & Edge Processing Machines",
      "Planing Machines",
      "Presses",
      "Woodworking Saws",
      "Wood Finishing Equipment",
      "Other",
    ],
  },
  "Lab & Pharmaceutical Equipment": {
    description: "",
    categories: [
      "Analytical Instruments",
      "Lab Supplies",
      "Medical & Lab Equipment",
      "Pharmaceutical Equipment",
      "Pharmaceutical Mixing & Blending Equipment",
      "Other",
    ],
  },
  "Heavy Construction Equipment": {
    description: "plus Heavy Machinery, cranes, backhoes",
    categories: [
      "Boom Lifts",
      "Cranes",
      "Bulldozers",
      "Excavators",
      "Light Plants & Towers",
      "Loaders",
      "Skid Steers",
      "Surface Equipment",
      "Rollers",
      "Other",
    ],
  },
  "Trucks, Vehicles, Parts & Accessories": {
    description: "",
    categories: [
      "Aircraft",
      "ATVs",
      "Auto Parts & Accessories",
      "Cars",
      "Buses",
      "Classic Cars",
      "Commercial Trucks",
      "Watercraft",
      "Motorcycles",
      "Pickup Trucks",
      "Recreational Vehicles",
      "SUVs",
      "Trailers",
      "Vans",
      "Other",
    ],
  },
  "Farm Equipment & Machinery": {
    description: "",
    categories: [
      "Farm Machinery Attachments",
      "Fuel",
      "Grain Handling Equipment",
      "Harvest Equipment",
      "Hay & Forage Equipment",
      "Landscape & Commercial Lawncare",
      "Livestock Supplies",
      "Planters & Seeding",
      "Applicators & Sprayers",
      "Tractors",
      "Other",
    ],
  },
  "Plastics & Rubber Manufacturing Equipment": {
    description: "",
    categories: [
      "Compounding & Pelletizing Machines",
      "Extrusion",
      "Injection Molding",
      "Post Processing Machines",
      "Raw Materials & Additives",
      "Plastics & Rubber Support Equipment",
      "Other",
    ],
  },
  "Drilling & Mining Equipment": {
    description: "",
    categories: [
      "Crushers",
      "Forestry & Logging Equipment",
      "Oilfield Equipment",
      "Support Equipment",
      "Other",
    ],
  },
  "Computer & Electronics": {
    description: "",
    categories: [
      "Audio / Visual Equipment",
      "Phones",
      "Computers",
      "Networking & Communications Equipment",
      "Printers & Scanners",
      "Security & Surveillance Equipment",
      "Other",
    ],
  },
  "Printing Equipment": {
    description: "",
    categories: [
      "Commercial Printers & Presses",
      "Bindery and Finishing Equipment",
      "Consumables",
      "Pre-Press Equipment",
      "Other",
    ],
  },
  Firearms: { description: "", categories: ["All"] },
  "Fitness Equipment": { description: "", categories: ["All"] },
  "Home & Furniture": { description: "", categories: ["All"] },
  "Office furniture & supplies": { description: "", categories: ["All"] },
  "Recycling Equipment": { description: "", categories: ["All"] },
  "Retail Fixtures": { description: "", categories: ["All"] },
  "Real Estate & Land": { description: "", categories: ["All"] },
};

const industryCategoriesautocat = {
  "Agricultural equipment": {
    "description": "",
    "categories": [
      "Attachments",
      "Fuel",
      "Grain Handling",
      "Harvesting",
      "Hay & Forage",
      "Irrigation",
      "Landscaping",
      "Livestock & Livestock Handling",
      "Seeding & Tillage",
      "Sprayers & Spreaders",
      "Tractors"
    ]
  },
  "Automotive & Vehicles": {
    "description": "",
    "categories": [
      "Aircraft",
      "ATVs",
      "Auto Parts & Accessories",
      "Automobiles",
      "Buses",
      "Classics",
      "Heavy Duty Trucks",
      "Marine",
      "Motorcycles",
      "Pickup Trucks",
      "Recreational",
      "SUVs",
      "Trailers",
      "Vans"
    ]
  },
  "Computers & electronics": {
    "description": "",
    "categories": [
      "Audio Visual Equipment",
      "Cell Phones & PDAs",
      "Computers",
      "IT Accessories",
      "Networking & Communications",
      "Printers & Scanners",
      "Security & Surveillance Equipment"
    ]
  },
  "Drilling Mining & Forestry": {
    "description": "",
    "categories": [
      "Crushing & Conveying",
      "Drills",
      "Logging & Sawmill Equipment",
      "Oilfield Equipment",
      "Processing Equipment",
      "Support Equipment",
      "Surface Equipment & Transport",
      "Underground Equipment"
    ]
  },
  "Food & Beverage Equipment": {
    "description": "",
    "categories": [
      "CIP Systems",
      "Complete Plants",
      "Cookers & Cookware",
      "Food Preparation",
      "Furniture",
      "Kitchen Equipment",
      "Mixers & Blenders",
      "Processing",
      "Refrigeration",
      "Restaurant & Bar Equipment",
      "Tanks & Vessels",
      "Warewashing",
      "Warmers & Cabinets"
    ]
  },
  "Heavy construction equipment": {
    "description": "",
    "categories": [
      "Boom Lifts",
      "Cranes",
      "Dozers",
      "Excavators",
      "Lighting",
      "Loaders",
      "Rail Construction Equipment",
      "Skid Steers",
      "Surface Equipment",
      "Telehandlers"
    ]
  },
  "Industrial & commercial": {
    "description": "",
    "categories": [
      "Compressors",
      "Electrical Supplies",
      "Forklifts",
      "Generators",
      "Handling & Packaging",
      "Other Industrial Supplies & Equipment",
      "Pipes & Fittings",
      "Scissor Lifts",
      "Warehouse Equipment"
    ]
  },
  "Laboratory & pharmaceutical equipment": {
    "description": "",
    "categories": [
      "Analytical Instruments",
      "Laboratory Supplies",
      "Medical Equipment",
      "Pharmaceutical Machinery",
      "Processing Equipment"
    ]
  },
  "Marine & aviation": {
    "description": "",
    "categories": ["All"]
  },
  "Metalworking": {
    "description": "",
    "categories": [
      "EDM Electrical Discharge Machining",
      "Machine Tools & Tools",
      "Machines",
      "Measuring & Testing Equipment",
      "Presses",
      "Saws",
      "Sheet Metal Processing",
      "Surface Treatment Machinery",
      "Welders"
    ]
  },
  "Other industries": {
    "description": "",
    "categories": [
      "Arcade Coin-Op Machines",
      "Firearms",
      "Fitness Equipment",
      "Home & Furniture",
      "Office furniture & supplies",
      "Recycling Equipment",
      "Retail Fixtures"
    ]
  },
  "Plastics & rubber machinery": {
    "description": "",
    "categories": [
      "Compounding & Pelletizing",
      "Extrusion",
      "Injection Molding",
      "Post Processing Machines",
      "Raw Materials & Additives",
      "Rotomolding",
      "Support Equipment",
      "Thermoforming"
    ]
  },
  "Printing equipment": {
    "description": "",
    "categories": [
      "Bindery and Finishing",
      "Consumables",
      "Mailing Equipment",
      "Pre-Press Equipment",
      "Printers & Presses"
    ]
  },
  "Real estate": {
    "description": "",
    "categories": ["Commercial", "Land", "Residential"]
  },
  "Woodworking": {
    "description": "",
    "categories": [
      "CNC Woodworking Machinery",
      "Drilling & Doweling Machines",
      "Milling, Morticing & Turning",
      "Panel & Edge Processing Machines",
      "Planing",
      "Presses",
      "Saws",
      "Surface Treatment",
      "Veneer Processing"
    ]
  }
};


const artCategories = {
  "Fine Art": {
    description: "",
    categories: [
      "Drawings & Pastels",
      "Paintings & Mixed Media",
      "Fine Art Photography",
      "Prints & Multiples",
      "Sculptures",
      "Watercolour Paintings",
      "Other",
    ],
  },
  Collectables: {
    description: "plus Books, Posters, Toys",
    categories: [
      "Bank Notes",
      "Books & Periodicals",
      "Cameras & Photography Equipment",
      "Coins",
      "Entertainment Memorabilia",
      "Antique Kitchenalia",
      "Antique & Vintage Maps",
      "Toys & Games",
      "Musical Instruments & Memorabilia",
      "Railwayana",
      "Religious Items & Folk Art",
      "Antique Scientific Instruments",
      "Sports Memorabilia & Equipment",
      "Collectable Stamps",
      "Taxidermy",
      "Antique & Vintage Tools",
      "Vintage Wine & Spirits",
      "Writing Instruments",
      "Cameras",
      "Sports Autographs",
      "British Coins",
      "Vintage Guitars",
      "Sports Posters",
      "Other",
    ],
  },
  "Watches & Jewellery": {
    description: "",
    categories: [
      "Barometers",
      "Antique & Vintage Clocks",
      "Jewellery",
      "Watches",
      "Rings",
      "Brooches",
      "Pocket Watches",
      "Bracelets",
      "Earrings",
      "Other",
    ],
  },
  "Decorative Art": {
    description: "",
    categories: [
      "Antique & Vintage Ceramics",
      "Antique & Vintage Glassware",
      "Antique & Vintage Metalware",
      "Antique & Vintage Porcelain",
      "Antique & Vintage Silver",
      "Ceramic Vases",
      "Glass Vases",
      "Porcelain Plates",
      "Ceramic Pots",
      "Murano Glass",
      "Silver Vases",
      "Glass Bottles",
      "Glass Tumblers",
      "Glass Lamps",
      "Other",
    ],
  },
  Furniture: {
    description: "",
    categories: [
      "Rugs & Carpets",
      "Designer Furniture - Modern & Contemporary",
      "Lighting",
      "Salvage & Architectural Antiques",
      "Tables",
      "Chairs",
      "Cabinets & Case Pieces",
      "Sofas",
      "Dining Tables",
      "Beds",
      "Other",
    ],
  },
  "Asian Art": {
    description: "",
    categories: [
      "Chinese Art & Antiques",
      "Indian Art & Antiques",
      "Islamic Art & Antiques",
      "Japanese Art & Antiques",
      "Other",
    ],
  },
  "Arms, Armour & Militaria": {
    description: "",
    categories: [
      "Militaria",
      "Knives",
      "Armour",
      "Badges, Medals & Pins",
      "WW2 Memorabilia For Sale",
      "Antique Pistols",
      "Military Uniforms",
      "Swords",
      "Other",
    ],
  },
  "Classic Cars": {
    description: "Vehicles, parts and accessories",
    categories: ["Classic Cars", "Motorbikes", "Scooters", "Bicycles"],
  },
  "Vintage Fashion": {
    description: "",
    categories: ["Textiles", "Vintage Fashion", "Bags & Handbags"],
  },
};

commercialCategories = {
  Vehicles: {
    description: "",
    categories: [
      "4WD Vehicles",
      "Auto Parts",
      "Aviation & Marine",
      "Cars",
      "Collector & Classic Cars",
      "Motorbikes",
      "Trucks",
      "Vans & Commercial Vehicles",
      "Other",
    ],
  },
  Fashion: {
    description: "",
    categories: [
      "Bags",
      "Clothing",
      "Underwear",
      "Garments",
      "Shoes",
      "Sunglasses",
      "Shirts",
      "Tops",
      "Trainers",
      "Pants",
      "Other",
    ],
  },
  Electronics: {
    description: "",
    categories: [
      "Audio & Electronic Musical Equipment",
      "Phones",
      "Tablets",
      "Kitchen Appliances",
      "Small Appliances",
      "Gaming & Consoles",
      "Photography Equipment",
      "TVs",
      "Computer Accessories",
      "IT Networking & Infrastructure Equipment",
      "Computers",
      "Monitors",
      "Printers & Copiers",
      "Cameras",
      "Speakers",
      "Headphones",
      "Ovens & Cookers",
      "Washing Machines",
      "Guitars",
      "Fridges & Freezers",
      "Tumble Dryers",
      "Dishwashers",
      "Other",
    ],
  },
  Homeware: {
    description: "",
    categories: [
      "Mirrors",
      "Bathroom Fixtures & Fittings",
      "Kitchenware",
      "Bedrooms",
      "Children & Baby Products",
      "Office Furniture & Supplies",
      "Other",
    ],
  },
  "Home & Garden": {
    description: "",
    categories: [
      "Home Improvement",
      "Garden & Outdoors",
      "Tools",
      "Doors",
      "Flooring",
      "Garden Furniture",
      "Other",
    ],
  },
  "Jewellery & Watches": {
    description: "",
    categories: [
      "Bracelets",
      "Earrings",
      "Gemstones",
      "Necklaces",
      "Chains",
      "Rings",
      "Watches",
      "Other",
    ],
  },
  Consumables: {
    description: "",
    categories: [
      "Wine & Spirits",
      "Containers",
      "Pet Supplies",
      "Perfume & Cosmetics",
      "Other",
    ],
  },
  "Toys & Games": {
    description: "",
    categories: [
      "Remote-Controlled Toys",
      "Board Games",
      "Cards",
      "Dinosaur Toys",
      "Other",
    ],
  },
  Fitness: {
    description: "",
    categories: [
      "Golfing Equipment",
      "Fishing Gear & Equipment",
      "Football Equipment",
      "Boxing Equipment",
      "Treadmills",
      "Stationary Bicycles",
      "Bikes",
      "Other",
    ],
  },
  Industrial: {
    description: "",
    categories: [
      "Farm & Agricultural Equipment",
      "Food & Beverage Equipment",
      "Metalworking Machinery",
      "Heavy Construction Equipment",
      "Medical Equipment",
      "Woodworking Equipment",
      "Handling & Packaging Equipment",
      "Small Plant & Tools",
      "Restaurant & Bar Equipment",
      "Woodworking Tools",
      "Forklifts",
      "Excavators",
      "Tractors",
      "Other",
    ],
  },
};

alibabaCategories = {
  "Acura": {
    "description": "",
    "categories": ["MDX", "RDX", "TLX", "Integra", "NSX"]
  },
  "Alfa Romeo": {
    "description": "",
    "categories": ["Giulia", "Stelvio", "Tonale", "Giulietta"]
  },
  "Aston Martin": {
    "description": "",
    "categories": ["DB11", "DBS", "Vantage", "DBX"]
  },
  "Audi": {
    "description": "",
    "categories": ["A3", "A4", "A6", "Q5", "Q7", "Q8", "R8", "e-tron"]
  },
  "Bentley": {
    "description": "",
    "categories": ["Continental GT", "Flying Spur", "Bentayga"]
  },
  "BMW": {
    "description": "",
    "categories": ["3 Series", "5 Series", "7 Series", "X3", "X5", "X7", "M3", "M5"]
  },
  "Bugatti": {
    "description": "",
    "categories": ["Chiron", "Veyron", "Divo"]
  },
  "Buick": {
    "description": "",
    "categories": ["Enclave", "Encore", "Envision"]
  },
  "Cadillac": {
    "description": "",
    "categories": ["CT4", "CT5", "Escalade", "XT4", "XT5", "XT6"]
  },
  "Chevrolet": {
    "description": "",
    "categories": ["Silverado", "Equinox", "Tahoe", "Corvette", "Camaro", "Malibu"]
  },
  "Chrysler": {
    "description": "",
    "categories": ["300", "Pacifica", "Voyager"]
  },
  "Dodge": {
    "description": "",
    "categories": ["Charger", "Challenger", "Durango", "Hornet"]
  },
  "Ferrari": {
    "description": "",
    "categories": ["296 GTB", "SF90 Stradale", "Roma", "812 Superfast", "F8 Tributo"]
  },
  "Fiat": {
    "description": "",
    "categories": ["500", "500X", "Panda", "Tipo"]
  },
  "Ford": {
    "description": "",
    "categories": ["F-150", "Explorer", "Mustang", "Bronco", "Escape", "Maverick"]
  },
  "Genesis": {
    "description": "",
    "categories": ["G70", "G80", "G90", "GV70", "GV80"]
  },
  "GMC": {
    "description": "",
    "categories": ["Sierra", "Yukon", "Acadia", "Terrain", "Hummer EV"]
  },
  "Honda": {
    "description": "",
    "categories": ["Accord", "Civic", "CR-V", "Pilot", "Odyssey", "Ridgeline"]
  },
  "Hyundai": {
    "description": "",
    "categories": ["Elantra", "Sonata", "Tucson", "Santa Fe", "Palisade", "Kona"]
  },
  "Infiniti": {
    "description": "",
    "categories": ["Q50", "Q60", "QX50", "QX60", "QX80"]
  },
  "Jaguar": {
    "description": "",
    "categories": ["F-PACE", "E-PACE", "I-PACE", "XF", "F-TYPE"]
  },
  "Jeep": {
    "description": "",
    "categories": ["Wrangler", "Grand Cherokee", "Cherokee", "Compass", "Gladiator"]
  },
  "Kia": {
    "description": "",
    "categories": ["Forte", "K5", "Sorento", "Sportage", "Telluride", "Soul"]
  },
  "Lamborghini": {
    "description": "",
    "categories": ["Urus", "Huracán", "Revuelto", "Aventador"]
  },
  "Land Rover": {
    "description": "",
    "categories": ["Range Rover", "Defender", "Discovery", "Range Rover Sport", "Range Rover Evoque"]
  },
  "Lexus": {
    "description": "",
    "categories": ["ES", "IS", "LS", "RX", "NX", "GX", "LX"]
  },
  "Lincoln": {
    "description": "",
    "categories": ["Navigator", "Aviator", "Corsair", "Nautilus"]
  },
  "Maserati": {
    "description": "",
    "categories": ["Ghibli", "Levante", "Quattroporte", "MC20", "Grecale"]
  },
  "Mazda": {
    "description": "",
    "categories": ["Mazda3", "Mazda6", "CX-5", "CX-50", "CX-9", "MX-5 Miata"]
  },
  "McLaren": {
    "description": "",
    "categories": ["720S", "Artura", "GT", "765LT"]
  },
  "Mercedes-Benz": {
    "description": "",
    "categories": ["C-Class", "E-Class", "S-Class", "GLC", "GLE", "GLS", "G-Class", "AMG GT"]
  },
  "Mini": {
    "description": "",
    "categories": ["Cooper", "Countryman", "Clubman"]
  },
  "Mitsubishi": {
    "description": "",
    "categories": ["Outlander", "Eclipse Cross", "Mirage"]
  },
  "Nissan": {
    "description": "",
    "categories": ["Altima", "Sentra", "Rogue", "Pathfinder", "Titan", "Z"]
  },
  "Pagani": {
    "description": "",
    "categories": ["Huayra", "Utopia", "Zonda"]
  },
  "Porsche": {
    "description": "",
    "categories": ["911", "Cayenne", "Macan", "Panamera", "Taycan", "718 Cayman"]
  },
  "Ram": {
    "description": "",
    "categories": ["1500", "2500", "3500", "ProMaster"]
  },
  "Rolls-Royce": {
    "description": "",
    "categories": ["Phantom", "Ghost", "Cullinan", "Spectre"]
  },
  "Subaru": {
    "description": "",
    "categories": ["Outback", "Forester", "Crosstrek", "Ascent", "Impreza", "WRX"]
  },
  "Tesla": {
    "description": "",
    "categories": ["Model S", "Model 3", "Model X", "Model Y", "Cybertruck"]
  },
  "Toyota": {
    "description": "",
    "categories": ["Camry", "Corolla", "RAV4", "Highlander", "Tacoma", "Tundra", "Supra"]
  },
  "Volkswagen": {
    "description": "",
    "categories": ["Golf", "Jetta", "Passat", "Tiguan", "Atlas", "ID.4"]
  },
  "Volvo": {
    "description": "",
    "categories": ["S60", "S90", "XC40", "XC60", "XC90"]
  }
};

LiveAuctioneersCategories = {
  "Art": {
    "description": "",
    "categories": [
      "Drawings",
      "Masks",
      "Mixed Media & Collages",
      "Paintings",
      "Photography",
      "Prints & Multiples",
      "Scrolls, Screens & Fan Paintings",
      "Sculptures & Carvings",
      "Tribal & Native American Artifacts"
    ]
  },
  "Collectibles": {
    "description": "",
    "categories": [
      "Advertisements & Signs",
      "Banks, Registers & Vending Machines",
      "Books, Magazines & Papers",
      "Breweriana & Beer Collectibles",
      "Cameras & Photo Equipment",
      "Cars & Motors",
      "Casino & Gambling Collectibles",
      "Coins, Currency & Stamps",
      "Comic Books",
      "Entertainment Memorabilia",
      "Historical Memorabilia",
      "Militaria & War Memorabilia",
      "Musical Instruments",
      "Pens & Writing Instruments",
      "Radios, Phones & Electronics",
      "Religious & Spiritual Objects",
      "Rocks, Minerals & Fossils",
      "Scientific & Medical Objects",
      "Sewing, Weaving & Knitting",
      "Sports Equipment",
      "Sports Memorabilia",
      "Tobacciana",
      "Tools & Toolboxes",
      "Toys & Hobbies",
      "Transportation & Travel Collectibles",
      "Weapons & Armor",
      "Wine & Spirits"
    ]
  },
  "Fashion": {
    "description": "",
    "categories": [
      "Accessories & Vanity Items",
      "Bags & Purses",
      "Men's Clothing",
      "Shoes",
      "Women's Clothing"
    ]
  },
  "Furniture": {
    "description": "",
    "categories": [
      "Beds & Headboards",
      "Cabinets & Storage",
      "Seating",
      "Tables"
    ]
  },
  "Home & Decor": {
    "description": "",
    "categories": [
      "Architectural & Garden Elements",
      "Decorative Objects",
      "Kitchenware",
      "Lighting",
      "Rugs & Carpets",
      "Tableware & Barware",
      "Textiles & Linens"
    ]
  },
  "Jewelry": {
    "description": "",
    "categories": [
      "Body Jewelry",
      "Bracelets",
      "Brooches & Pins",
      "Cufflinks, Tie Pins & Tie Clips",
      "Earrings",
      "Jewelry Sets",
      "Loose Stones & Beads",
      "Necklaces & Pendants",
      "Rings",
      "Watches"
    ]
  }
};


module.exports = { industryCategoriesSEO, industryCategoriesMeganav, industryCategoriesautocat, artCategories, commercialCategories, alibabaCategories, LiveAuctioneersCategories };
